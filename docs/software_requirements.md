...
| SRS-MEM-001 | The demo shall write each completed burst to a memory-style record store. | `simulator/device_memory.py` receives each completed burst. |
| SRS-MEM-002 | Each log record shall include sequence, timestamp, sample count, CRC32, and sample payload. | `simple_acquisition_demo.py` writes this record header. |
